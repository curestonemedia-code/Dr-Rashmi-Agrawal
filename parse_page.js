...
function htmlToJsx(html) {
    ...
    jsx = jsx.replace(/<i data-lucide="([^"]+)"([^>]*)><\/i>/g, (match, icon, props) => {
        ...
        return `<Icons.${compName} ${props} />`;
    });
    return jsx;
}

const pages = [
    { file: 'index.html', out: 'src/app/page.tsx', comp: 'Home' },
    { file: 'kidney-stones.html', out: 'src/app/kidney-stones/page.tsx', comp: 'KidneyStones' },
    { file: 'prostate.html', out: 'src/app/prostate/page.tsx', comp: 'Prostate' },
    { file: 'andrology.html', out: 'src/app/andrology/page.tsx', comp: 'Andrology' }
];

for (const page of pages) {
    const html = fs.readFileSync(`legacy/${page.file}`, 'utf-8');
    const match = html.match(/<!-- .*HERO -->([\s\S]*?)<!-- FOOTER -->/i) || html.match(/<!-- .*HERO -->([\s\S]*)/i);
    if (!match) continue;
    
    let bodyText = match[1];
    bodyText = bodyText.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '');
    let body = htmlToJsx(bodyText);
    
    fs.mkdirSync(path.dirname(page.out), { recursive: true });
    
    const isHome = page.comp === 'Home';
    const homeLogic = `
        gsap.to('#hero-img', {
...
export default function ${page.comp}() {
  ...
  return (
    ...
    fs.writeFileSync(page.out, code);
}
